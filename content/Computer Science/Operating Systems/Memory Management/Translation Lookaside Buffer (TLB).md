...
---
> [!abstract] Abstract
> Without hardware caching, [[Operating Systems/Memory Management/Paging & Page Tables|paging]] imposes a severe performance penalty on memory operations because translating a virtual address requires accessing physical memory multiple times to walk [[Operating Systems/Memory Management/Multi-Level Page Tables|multi-level page tables]]. The **Translation Lookaside Buffer (TLB)** is a small, highly associative hardware cache built directly into the MMU that stores recent address translations, allowing virtual addresses to be translated to physical addresses in roughly a single CPU clock cycle.
> 
...
*   **Linear Page Tables:** Requires **2 memory accesses** (1 access to read the [[Operating Systems/Memory Management/Page Table Entries & Memory Overhead|Page Table Entry]], plus 1 access to read the actual data).
...
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------ |
| **Hardware-Managed (x86 / ARM)**    | The MMU hardware walks page tables directly in RAM via the [[Operating Systems/Memory Management/Kernel Address Space & Page Table Storage\|CR3 control register]] and updates the TLB. | Fast miss handling; no trap or context switch overhead.                      | Inflexible; page table structure is fixed by hardware. |
...
1.  **Flush Entire TLB:** Invalidate all TLB entries on every [[Operating Systems/Kernel & Architecture/Thread/Thread Context Switch & Scheduling|context switch]]. Simple, but causes a wave of TLB misses when the new process starts executing.
