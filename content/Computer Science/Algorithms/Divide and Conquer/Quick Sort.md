...
---
> [!abstract]
> Quick Sort picks a random pivot, partitions the list around it, and recursively sorts both sides — the randomized counterpart to [[Merge Sort]]'s deterministic split.
> 
...
> [!note] The argument below is added, following the same shape as [[Merge Sort#Proof of Correctness|Merge Sort's proof]].
...
- **$O(n^2)$ worst case**, unlike Merge Sort's guaranteed $\Theta(n\log n)$ — see [[Merge Sort]] when a worst-case guarantee matters more than average speed.
...
- **Alternatives to consider:** [[Merge Sort]] for a guaranteed worst case; Insertion Sort for small subarrays (often used as a cutoff inside real Quick Sort implementations, same as with Merge Sort).
