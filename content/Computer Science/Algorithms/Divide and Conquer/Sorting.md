...
A **comparison-based** sorting algorithm only learns information about the input by comparing pairs of elements (e.g. "is $a < b$?") — it never inspects or exploits the actual values otherwise. [[Merge Sort]] and [[Quick Sort]] are both comparison-based.
...
So **no comparison-based sorting algorithm can do better than $\Omega(n\log n)$ in the worst case.** This is why [[Merge Sort]]'s $\Theta(n\log n)$ worst-case bound is considered _optimal_ — it's not just a good algorithm, it's asymptotically as good as any comparison sort can ever be.
...
See [[Computer Science/Algorithms/Divide and Conquer/index#Deterministic vs. Randomized Approaches|Deterministic vs. Randomized Approaches]] for the full table including [[Selection]].
...
|---|---|
|[[Merge Sort]]|Divide and conquer sort — splits, recursively sorts each half, merges the sorted halves; $\Theta(n\log n)$ worst case, matching the comparison lower bound exactly|
