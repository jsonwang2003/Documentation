---
description: "A self-balancing binary search tree derived from 2-4 trees that maintains balance in a single pass using simple coloring rules and rotations."
aliases:
  - Red-Black Tree
  - RB Tree
  - Single-Pass Balanced Tree
tags:
  - data-structures
  - trees
  - red-black
---
> [!abstract] Abstract 
> Derived from the 2-4 Tree, the Red-Black Tree is a self-balancing [[Tree Structures/Binary Search Tree|Binary Search Tree]] that achieves a worst-case $O(\log n)$ time complexity. Unlike the [[Tree Structures/AVL Tree|AVL Tree]], which requires two passes (down and up) for updates, the Red-Black Tree is designed to maintain balance in a single pass through the tree.
> 
...
> - **Primary Advantage:** Faster insertions and deletions due to fewer total rotations during mutations.
...
# Insertion Algorithm: The Single-Pass Method
...
1. Perform a Single [[Tree Structures/AVL Tree|AVL Rotation]].
...
# Performance Trade-offs: AVL vs. Red-Black
...
> [!tip] Standard Library Default
> This efficiency in write-heavy scenarios is why Red-Black Trees are used as the backbone for standard library associative containers like `std::map` and `std::set` in C++.

---

# Summary of BST Evolution

| Data Structure     | Best Case   | Worst Case  | Balance Strategy                                                    |
| ------------------ | ----------- | ----------- | ------------------------------------------------------------------- |
| **Basic BST**      | $O(\log n)$ | $O(n)$      | None                                                                |
| **Randomized BST** | $O(\log n)$ | $O(n)$      | Random Priorities ([[Randomized Search Trees (Treap, RST)\|Treap]]) |
