...
> - **Core Usage:** Backbone for [[Tree Structures/Binary Search Tree|Binary Search Trees]], [[Tree Structures/Heap|Heaps]], and expression syntax trees.
...
| **Degenerate (Skewed) Tree** | Every internal node has only one child, causing the tree to degrade into a linear [[Computer Science/Data Structures/Introductory Data Structures/Linked List\|Linked List]] |
...
Visits nodes horizontally level by level from top to bottom, left to right. This algorithm utilizes a [[Introductory Data Structures/Queues|Queue]] data structure to track frontier nodes.
...
1.  **Dynamic Pointer-Based Nodes:**
    Nodes hold a data value alongside dynamic heap pointers (`leftChild`, `rightChild`, optional `parent`). This is the default structure for general dynamic trees like [[Tree Structures/AVL Tree|AVL Trees]] and [[Tree Structures/Binary Search Tree|BSTs]].
2.  **Array-Based Contiguous Index Mapping:**
    Used when the tree satisfies the Complete Tree property (such as [[Tree Structures/Heap|Binary Heaps]]). Left and right children map to array offsets via constant index arithmetic ($2i+1$ and $2i+2$), bypassing pointer storage overhead entirely.
