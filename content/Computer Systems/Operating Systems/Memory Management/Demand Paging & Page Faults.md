...
> - **Key Hardware Primitive:** Valid / Present bit in the [[Page Table Entries & Memory Overhead|Page Table Entry]].
...
When an application accesses a virtual page stored in the swap file, the hardware [[Page Table Entries & Memory Overhead|PTE valid bit]] is `0`. This triggers a hardware exception called a **Page Fault**.
...
5.  **PTE Update:** The OS updates the PTE with the physical frame number, sets `Valid = 1`, and updates the [[Translation Lookaside Buffer (TLB)|TLB]].
...
*   **Page Eviction & Dirty Bit:**
    ...
    *   **Clean Pages:** If the [[Page Table Entries & Memory Overhead|PTE Dirty Bit]] is `0` (unmodified), the OS discards the in-memory page without writing to disk because the disk copy is already identical.
...
| **Protection Fault**       | `Valid = 1`, but operation violates permissions (e.g., write to read-only page). | Abort process or execute [[Advanced Virtual Memory Features\|Copy-on-Write]] page replication. |
...
1.  Query the [[Translation Lookaside Buffer (TLB)|TLB]] for the target VPN (TLB Miss).
...
- [[Advanced Virtual Memory Features|Advanced Virtual Memory Features]]
