...
---
> [!abstract] Abstract
> A **Page Table Entry (PTE)** is a hardware-readable data structure that stores the physical mapping and operational status of a single virtual page. Flat, single-level linear page tables store an array of PTEs for every possible page in a process's [[Virtual Memory & Address Translation Fundamentals|Virtual Address Space]]. While simple, linear page tables suffer from severe memory overhead in large address spaces, an issue that cannot be solved by **Huge Pages** alone.
> 
...
When using [[Paging|paging]], each process maintains its own independent page table:
...
While huge pages improve Translation Lookaside Buffer (TLB) performance for memory-intensive applications, solving the page table memory size problem requires **hierarchical indirection** via [[Multi-Level Page Tables|Multi-Level Page Tables]].
