...
- **[[Process Lifecycle & API#Process Creation Models Windows vs. Unix|Process Creation]]**: Mastering the `fork()` system call to create exact duplicates of a running program.
...
- **[[Advanced Virtual Memory Features#Copy-on-Write (CoW)|Copy-on-Write]]**: The optimization trick that makes `fork()` efficient by sharing physical RAM until a write occurs.
