...
> 
> From [[7. Dijkstra's Algorithm#Drawbacks / Constraints|Dijkstra's]], one of its main constraints is that it only works with non-negative edge weights.
> 
...
|       | Complexity                      | Notes                                                                                                                                               |
| ----- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Time  | $O(\|V\|E\|)$                   | Substantially slower than [[7. Dijkstra's Algorithm\|Dijkstra's Algorithm]]'s $O((\|V\|+\|E\|)\log\|V\|)$ — the cost of tolerating negative weights |
...
- **Much slower than Dijkstra's.** Only use Bellman-Ford when negative edge weights are actually possible — if all weights are known non-negative, [[7. Dijkstra's Algorithm|Dijkstra's Algorithm]] is strictly better.
...
- [[5. Shortest Path in a DAG Example|Shortest Path in a DAG]]