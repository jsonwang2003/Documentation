...
> [!abstract] 
> **What it is:** Prim's Algorithm is a [[Computer Science Introduction/Algorithms/II. Greedy Algorithms/index|Greedy Algorithm]] that builds a **[[Minimum Spanning Trees|Minimum Spanning Tree (MST)]]** for a connected, undirected, weighted graph by repeatedly growing a single tree one cheapest edge at a time.
> 
...
> 
> Update a vertex by putting the **lowest cost vertex** from $F$ into $X$, then re-check its neighbors: if a neighbor now has a cheaper bridge through this new tree vertex, update its `cost` (same "relax" idea as [[7. Dijkstra's Algorithm|]], just comparing one edge weight instead of a cumulative path).

---
# Pseudocode (Mid-Level Implementation)

Similar to [[7. Dijkstra's Algorithm|Dijkstra's Algorithm]], **Prim's** uses the value $cost(v)$ instead of $dist(v)$ — same loop, same priority queue, but the relaxation compares just the single edge weight $\ell(v,u)$ against `cost(u)`, not a cumulative path length.
...
- **Maintenance:** Suppose the invariant holds — $X$'s edges so far are consistent with some MST $T$. Consider the cut $(X, V-X)$. By the [[10. Cut Property|Cut Property]], the minimum-weight edge crossing this cut is guaranteed to be in _some_ MST. Prim's always selects exactly this edge next — the vertex $v \in F$ with lowest `cost(v)` is, by construction, the endpoint of the cheapest edge crossing the cut $(X, V-X)$ (since `cost(v)` was set to the weight of the cheapest edge from $X$ to $v$ during relaxation). So adding $(v, prev(v))$ to `output` keeps the invariant true — either $T$ already contains this edge, or swapping it into $T$ (removing whatever edge $T$ used to connect $v$) produces another MST of equal or lower weight, since the swapped-in edge is minimum-weight across the cut. See [[Cut Property]] for the full proof of why this swap argument works.
...
- **Not suitable for:** Finding shortest paths between vertices — an MST minimizes total tree weight, not pairwise path weight; use [[7. Dijkstra's Algorithm|Dijkstra's Algorithm]] (non-negative weights) or Bellman-Ford (negative weights allowed) for shortest paths instead.
...
- **Alternatives to consider:** [[8. Kruskal's Algorithm|Kruskal's Algorithm]] — a different greedy MST algorithm that sorts all edges globally and uses union-find, often preferable for very sparse graphs or when edges are already sorted/streamed.
