...
- **Maintenance:** Consider an edge $(u,v)$ with $find(u) \neq find(v)$. Let $C$ be $u$'s current component (as tracked by the disjoint-set structure). The cut $(C, V-C)$ separates $u$ from $v$, and $(u,v)$ crosses it. Because edges are processed in increasing weight order and no earlier (cheaper) edge crossing this cut has been added — otherwise `find(u)` and `find(v)` would already agree — $(u,v)$ is the cheapest edge crossing $(C, V-C)$ seen so far. By the [[10. Cut Property|Cut Property]], this edge belongs to some MST, so adding it keeps $X$ a subset of some MST.
...
- **Not suitable for:** Very dense graphs, where the $O(|E|\log|E|)$ sort becomes expensive relative to Prim's array-based $O(|V|^2)$ — use [[9. Prim's Algorithm|Prim's Algorithm]] instead when $|E| = \Theta(|V|^2)$.
- **Alternatives to consider:** [[9. Prim's Algorithm|Prim's Algorithm]] for dense graphs or when growing a single connected tree incrementally is more natural (e.g. streaming vertices rather than a static edge list).
