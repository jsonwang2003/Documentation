...
> 
> - **Category:** Network Flow / Combinatorial Optimization (also formulable as [[Computer Science Introduction/Algorithms/VI. Linear Programming/index|Linear Programming]])
...
- **Find an $s$-$t$ path in the residual graph** — [[3. Depth First Search (DFS)|Depth First Search (DFS)]] (as specified), $O(|V|+|E|)$ per call.
...
- **Pseudo-polynomial runtime.** $O(W|V||E|)$ depends on the _numeric value_ of capacities, not just the graph's size — with large capacities, this can be slow, matching the same caveat seen in [[3. The Knapsack Problem Example|Knapsack Problem]]'s $O(nC)$.
