...
title: Techniques to Prove Optimality
---
...
Take an arbitrary solution $OS$ that skips the greedy algorithm's first choice $g$, and show how to _exchange_ one element of $OS$ for $g$ to build a new solution $OS'$ that is still valid and at least as good. Then induct on **instance size**. See [[2. Event Scheduling Example]] for the full worked example.
...
- [[2. Event Scheduling Example]] — worked example for both Exchange Argument and Greedy Stays Ahead
- [[4. Event Scheduling with Multiple Rooms]] — worked example for Achieves the Bound (same problem as [[4. Event Scheduling with Multiple Rooms]] in this vault)
- [[5. Prove Kruskal's with Exchange Argument]]