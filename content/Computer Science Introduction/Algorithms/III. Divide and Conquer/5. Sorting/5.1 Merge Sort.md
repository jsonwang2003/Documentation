...
|       | Complexity                                                    | Notes                                                                                                                                      |
| ----- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Time  | $O(n\log n)$ — worst, best, and average case are all the same | The split is always exactly in half regardless of input, so there's no "unlucky" input the way there is for [[5.2 Quick Sort\|Quick Sort]] |
...
- **Not in-place.** Requires $O(n)$ auxiliary space for the merge step, unlike [[5.2 Quick Sort|Quick Sort]]'s partition, which can be done with $O(1)$ extra space (see [[Computer Science Introduction/Algorithms/III. Divide and Conquer/4. Selection/index#Partition with Pivot (Core Subroutine)|Partition with Pivot]]).
- **Not adaptive.** Runs in $\Theta(n\log n)$ even on already-sorted input — algorithms like Insertion Sort can detect and exploit partial sorted-ness to run faster on nearly-sorted data, but Merge Sort always does the same amount of work.
...
- **Alternatives to consider:** [[5.2 Quick Sort|Quick Sort]] when average-case speed matters more than worst-case guarantees; Insertion Sort for small or nearly-sorted inputs (often used as the base case inside a real Merge Sort implementation once the array is small enough).

---

# References / Links

- [[Computer Science Introduction/Algorithms/III. Divide and Conquer/5. Sorting/index|Soritng]]
- [[5.2 Quick Sort|Quick Sort]]
- [[Master Theorem]]