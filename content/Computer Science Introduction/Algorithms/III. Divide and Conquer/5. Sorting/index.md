...
---
> [!abstract] Overview 
> Before diving into specific sorting algorithms like [[5.1 Merge Sort|Merge Sort]] and [[5.2 Quick Sort|Quick Sort]], it's worth establishing _how fast sorting can possibly be_ — every comparison-based sorting algorithm is bound by the same $\Omega(n\log n)$ limit, which comes from a simple counting argument, not from any particular algorithm's design.
...
A **comparison-based** sorting algorithm only learns information about the input by comparing pairs of elements (e.g. "is $a < b$?") — it never inspects or exploits the actual values otherwise. [[5.1 Merge Sort|Merge Sort]] and [[5.2 Quick Sort|Quick Sort]] are both comparison-based.
...
So **no comparison-based sorting algorithm can do better than $\Omega(n\log n)$ in the worst case.** This is why [[5.1 Merge Sort|Merge Sort]]'s $\Theta(n\log n)$ worst-case bound is considered _optimal_ — it's not just a good algorithm, it's asymptotically as good as any comparison sort can ever be.
...
| ----------- | --------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **Sorting** | [[5.1 Merge Sort\|Merge Sort]] — $O(n\log n)$ | [[5.2 Quick Sort\|Quick Sort]] — Best: $O(n\log n)$, Worst: $O(n^2)$, Average: $O(n\log n)$ |

See [[Computer Science Introduction/Algorithms/III. Divide and Conquer/index#Deterministic vs. Randomized Approaches|Deterministic vs. Randomized Approaches]] for the full table including [[Computer Science Introduction/Algorithms/III. Divide and Conquer/4. Selection/index|Selection]].
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [[5.1 Merge Sort\|Merge Sort]] | Divide and conquer sort — splits, recursively sorts each half, merges the sorted halves; $\Theta(n\log n)$ worst case, matching the comparison lower bound exactly |
| [[5.2 Quick Sort\|Quick Sort]] | Divide and conquer sort — partitions around a pivot, recursively sorts each side; $O(n\log n)$ expected, $O(n^2)$ worst case                                       |
