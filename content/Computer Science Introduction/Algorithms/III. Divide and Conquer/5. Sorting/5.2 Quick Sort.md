...
---
> [!abstract]
> Quick Sort picks a random pivot, partitions the list around it, and recursively sorts both sides — the randomized counterpart to [[5.1 Merge Sort]]'s deterministic split.
> 
...
> [!tip] Key Idea 
> This is the same partition idea as [[4.1 Quick Select]] and [[Computer Science Introduction/Algorithms/III. Divide and Conquer/4. Selection/index#Partition with Pivot|Partition with Pivot]] — but where QuickSelect only ever recurses into _one_ side (since it just needs one rank), Quick Sort must recurse into **both** sides, since every element needs to end up in its correct position, not just the one at rank $k$.
...
- **Partition around `a_i`** — same idea as [[Computer Science Introduction/Algorithms/III. Divide and Conquer/4. Selection/index#Partition with Pivot|Partition with Pivot]]; can be done in place in $O(n)$ time.
...
> [!note] The argument below is added, following the same shape as [[5.1 Merge Sort#Proof of Correctness|Merge Sort's proof]].
...
> [!Important] 
> Like [[4.2. Deterministic Selection]]'s recurrence, this can't be solved with the [[Master Theorem]] directly — it's a **full-history recurrence** (it depends on _every_ smaller subproblem size, not just $n/2$ or a fixed fraction of $n$). The standard way to close this: guess $ET(n) \leq an\ln n$ for a suitable constant $a$, substitute the guess back into the sum, bound $\sum_{k=1}^{n-1} k\ln k$ using the integral $\int_1^n x\ln x,dx = \frac{n^2}{2}\ln n - \frac{n^2}{4}$, and verify the resulting expression is $\leq an\ln n$ for large enough $a$ — completing the induction and confirming $ET(n) = O(n\log n)$.
...
- **$O(n^2)$ worst case**, unlike Merge Sort's guaranteed $\Theta(n\log n)$ — see [[5.1 Merge Sort]] when a worst-case guarantee matters more than average speed.
...
- **Alternatives to consider:** [[5.1 Merge Sort]] for a guaranteed worst case; Insertion Sort for small subarrays (often used as a cutoff inside real Quick Sort implementations, same as with Merge Sort).

---

# References / Links

- [[Computer Science Introduction/Algorithms/III. Divide and Conquer/5. Sorting/index]]
- [[5.1 Merge Sort]]
- [[Computer Science Introduction/Algorithms/III. Divide and Conquer/4. Selection/index]]
- [[4.1 Quick Select]]