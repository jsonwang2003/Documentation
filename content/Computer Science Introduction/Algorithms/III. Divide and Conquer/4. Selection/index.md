...
Given a list $L = [a_1, \dots, a_n]$ and a pivot $a_i$, rearrange $L$ so that all elements smaller than $a_i$ are to the left of $a_i$ and all elements larger are to the right. This is the core operation both [[4.1 Quick Select|Quick Select]] and [[4.2. Deterministic Selection|Deterministic Selection]] are built on.
...
- [[4.1 Quick Select|Quick Select]] — picks the pivot **randomly**; simple, expected $O(n)$ time, but $O(n^2)$ worst case.
- [[4.2. Deterministic Selection|Deterministic Selection]] — picks the pivot via a guaranteed-good strategy (median-of-medians); worst-case $O(n)$ time, at the cost of a larger constant factor.
...
The complexity of the _full_ selection algorithm depends entirely on how the pivot is chosen — see [[4.1 Quick Select|Quick Select]] and [[4.2. Deterministic Selection|Deterministic Selection]] for those analyses.

## Deterministic vs. Randomized

|               | Deterministic                                                                          | Randomized                                                                          |
| ------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **Selection** | [[4.2. Deterministic Selection\|Deterministic Selection]] (Median of Medians) — $O(n)$ | [[4.1 Quick Select\|Quick Select]] — Best: $O(n)$, Worst: $O(n^2)$, Average: $O(n)$ |

See [[Computer Science Introduction/Algorithms/III. Divide and Conquer/index#Deterministic vs. Randomized Approaches|Deterministic vs. Randomized Approaches]] for the full table including [[Computer Science Introduction/Algorithms/III. Divide and Conquer/5. Sorting/index|Sorting Algorithms]].
...
- **Pivot choice matters enormously.** A poor pivot (e.g. always the min or max) barely shrinks the problem each recursive call — see [[4.1 Quick Select|Quick Select]]'s worst-case analysis for exactly how bad this gets.
...
- [[4.1 Quick Select|Quick Select]]
- [[4.2. Deterministic Selection|Deterministic Selection]]
