...
---
> [!abstract]
> Kruskal's Algorithm is a [[Computer Science Introduction/Algorithms/Greedy Algorithms/index|Greedy Algorithm]] that builds a **Minimum Spanning Tree (MST)** by sorting all edges by weight and adding each one that doesn't create a cycle, using a disjoint-set (union-find) structure to check for cycles in $O(\alpha(V))$ per edge.
> 
